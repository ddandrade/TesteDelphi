{
  Teste para vaga de Analista Delphi - WK Consultoria
  Autor candidato: Daniel Delfino Andrade
}

{ REQUISITOS DO TESTE }
{ DONE -oDaniel -cProjeto:Desenvoler tela de pedido de vende }
{ DONE -oDaniel -cProjeto: disponibilizar botão, que deve ser exibido quando o código do cliente estiver em branco, para carregar pedidos já gravados.
  Solicitar o número do pedido e carregar o cliente e os produtos }
{ DONE -oDaniel -cProjeto: disponibilizar botão, que deve ser exibido quando o código do cliente estiver em branco, para cancelar/apagar um pedido,
  excluindo seus respectivos dados nas entidades: dados_gerais_pedido e produtos_pedido }
{ DONE -oDaniel -cPedido: exibir no rodapé da tela o valor total do pedido }
{ DONE -oDaniel -cPedido: pedido deve possuir número sequencial crescente }
{ DONE -oDaniel -cPedido: incluir botão gravar que, quando acinado, o dados deverão ser persistidos em dados_gerais_pedido e produtos_pedito }
{ DONE -oDaniel -cItem Pedido: para inserir um item o usuário deverá informar código do produto, quantidade e valor unitário }
{ DONE -oDaniel -cItem Pedido: produto confirmado deve ser inseriro no grid do pedido; deve existir um botão para executar ação }
{ DONE -oDaniel -cItem Pedido: o grid deve apresentar: código, descrição, quantidade, valor unitário e total }
{ DONE -oDaniel -cItem Pedido: permitir navegação pelos itens com setas para cima e para baixo }
{ DONE -oDaniel -cItem Pedido: navegando pelo grid, deve ser permitida acesso a edição do item selecionado ao pressionar tecla "enter".
  A edição deve permitir alterar quantidade e valor unitário.
  Utilizar o mesmo botão de inserir para confirmar a alteração }
{ DONE -oDaniel -cItem Pedido: permitir a exclusão do item ao pressionar a tecla "DEL" }
{ DONE -oDaniel -cItem Pedido: antes de exclluir um item, solicitar confirmação do usuário }
{ DONE -oDaniel -cItem Pedido: permitir produtos repetidos no grid }

{ DONE -oDaniel -cPersistência: utilizar de transações e tratar possíveis erros }
...
begin
  inherited;
  { DONE -oDaniel -cInteface: método para Apagar }
  ExcluirUmPedido;
end;
...
begin
  inherited;
  EditarPedido;
  { DONE -oDaniel -cInterface:Criar método para edição do pedido }
end;
...
begin
  ...
  { DONE -oDaniel -cSalvar: criar método para Salvar pedido }
  Try
...
begin
  Status := FListaPedidos.Status;
  Try
    { DONE -oDaniel -cInteface: método para colocar pedido em modo edição }
...
begin
  ...
  begin
    Try
       //TENTA LOCALICAR O PEDIDO NO GRID
...
begin
  ...
  begin
    ...
    begin
      ...
      edtNomeCliente.Text := oPedido.Cliente.Nome;
      ExibirItensPedido(oPedido);
      // passando método anônimo inserir pedido.
    end;
...
begin
  ...
  case StatusFila of
    ...
    TAcaoLista.Navegacao:
      Begin
//        FListaPedidos.FocarPedido(FListaPedidos.ObterIndice(FListaPedidos.GetPedidoDestaque));
      end;
    TAcaoLista.Insercao:
      begin
      end;
...
begin
  inherited;
  { DONE -oDaniel -cDesenvolver: criar acesso a tela de pesquisa de cliente }
  Try
